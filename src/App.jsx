  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 dark:from-slate-900 dark:to-slate-800">
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6">
        ...
        <div className="mb-6 relative">
          <div className="relative">
            <input
              ...
              placeholder="🔍 Rechercher un joueur, match, date, ligue..."
              className="w-full px-4 py-3 pl-10 rounded-lg border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-800 dark:text-white placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:ring-2 focus:ring-purple-500 dark:focus:ring-purple-400"
            />
          ...
          {showSearchResults && globalSearchQuery.length > 0 && (
            <>
              ...
              <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 rounded-lg shadow-xl border border-slate-200 dark:border-slate-700 max-h-96 overflow-y-auto z-50">
                {(() => {
                  ...
                  const results = matchData.filter(match => {
                    return (
                      ...
                      `${match.buts_j1}-${match.buts_j2}`.includes(query)
                    );
                  });
                  ...
                  return (
                    <div className="p-2">
                      ...
                      {results.slice(0, 20).map((match, idx) => (
                        <div
                          key={idx}
                          className="px-3 py-3 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-lg cursor-pointer transition-colors"
                          onClick={() => {
                            setSelectedSeason(match.saison);
                        ...
                        >
                          ...
                          <div className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400">
                            <span>{match.dateMatch}</span>
                            <span>•</span>
                            <span>{match.ligue}</span>
                            <span>•</span>
                            <span>{match.championnat}</span>
                            <span>•</span>
                            <span className="text-purple-600 dark:text-purple-400">{match.saison}</span>
                          </div>
                        </div>
                      ))}
                      {results.length > 20 && (
                        <div className="px-3 py-2 text-center text-xs text-slate-500 dark:text-slate-400">
                          ... et {results.length - 20} autre{results.length - 20 > 1 ? 's' : ''} résultat{results.length - 20 > 1 ? 's' : ''}
                        </div>
                      )}
                    </div>
                  );
        ...
        {activeTab === 'test' && (
          <div className="space-y-6">
            <div className="bg-gradient-to-r from-purple-500 to-purple-700 rounded-xl shadow-sm p-6 text-white">
              <h2 className="text-2xl font-bold mb-2">🧪 Fonctionnalités Expérimentales</h2>
              <p className="text-purple-100">Nouvelles features en test : badges, prédictions, power ranking et analyses avancées</p>
            </div>

            {/* Badges System */}
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">🏆 Système de Badges</h3>
              <p className="text-sm text-slate-600 mb-4">Achievements débloqués par les joueurs</p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {joueurs.map(joueur => {
                  const playerMatches = filteredData
                    .filter(m => m.joueur1 === joueur || m.joueur2 === joueur)
                  ...
                  // Badge: Hat-trick hero (3+ consecutive wins)
                  let maxWinStreak = 0;
                  let currentStreak = 0;
                  ...
                  if (maxWinStreak >= 3) {
                    badges.push({ icon: '🔥', name: 'Hat-trick Hero', desc: `${maxWinStreak} victoires d'affilée` });
                  }

                  // Badge: Clean sheet king (5+ matches without conceding)
                  let cleanSheetStreak = 0;
                  let maxCleanSheetStreak = 0;
                  playerMatches.forEach(m => {
                    if (m.butsAdv === 0) {
                      cleanSheetStreak++;
                      maxCleanSheetStreak = Math.max(maxCleanSheetStreak, cleanSheetStreak);
                    } else {
                      cleanSheetStreak = 0;
                    }
                  });
                  if (maxCleanSheetStreak >= 5) {
                    badges.push({ icon: '🛡️', name: 'Clean Sheet King', desc: `${maxCleanSheetStreak} matchs sans encaisser` });
                  }

                  // Badge: Goal machine (10+ goals in any championship)
                  const goalsByChampionship = {};
                  filteredData.forEach(m => {
                    const key = `${m.ligue}-${m.championnat}`;
                    if (!goalsByChampionship[key]) goalsByChampionship[key] = 0;
                    if (m.joueur1 === joueur) goalsByChampionship[key] += m.buts_j1;
                    if (m.joueur2 === joueur) goalsByChampionship[key] += m.buts_j2;
                  });
                  const maxGoals = Math.max(...Object.values(goalsByChampionship), 0);
                  if (maxGoals >= 10) {
                    badges.push({ icon: '⚽', name: 'Goal Machine', desc: `${maxGoals} buts dans un championnat` });
                  }

                  // Badge: Comeback King (win after 2+ consecutive losses)
                  let lossStreak = 0;
                  let hadComeback = false;
                  playerMatches.forEach((m, idx) => {
                    if (m.result === 'L') {
                      lossStreak++;
                    } else if (m.result === 'W' && lossStreak >= 2) {
                      hadComeback = true;
                      lossStreak = 0;
                    } else {
                      lossStreak = 0;
                  ...
                  if (hadComeback) {
                    badges.push({ icon: '💪', name: 'Comeback King', desc: 'Victoire après 2+ défaites' });
                  }

                  return (
                    <div key={joueur} className="border border-slate-200 rounded-lg p-4">
                      <h4 className="font-bold text-lg text-slate-800 mb-3">{joueur}</h4>
                      {badges.length > 0 ? (
                        <div className="space-y-2">
                          {badges.map((badge, idx) => (
                            <div key={idx} className="flex items-center gap-3 bg-gradient-to-r from-yellow-50 to-yellow-100 border border-yellow-200 rounded-lg p-3">
                        ...
                        <p className="text-sm text-slate-500 italic">Aucun badge débloqué</p>
                      )}
                    </div>
                  );
                })}
            ...
            {/* Match Predictor */}
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">🔮 Prédicteur de Match</h3>
              <p className="text-sm text-slate-600 mb-4">Prédiction basée sur l'historique H2H et la forme récente</p>
              ...
              {(() => {
                // Calculate H2H stats
                const h2hMatches = filteredData.filter(m =>
                  (m.joueur1 === selectedVersusPlayer1 && m.joueur2 === selectedVersusPlayer2) ||
                  (m.joueur1 === selectedVersusPlayer2 && m.joueur2 === selectedVersusPlayer1)
                );
                ...
                h2hMatches.forEach(m => {
                  if (m.joueur1 === selectedVersusPlayer1) {
                    if (m.buts_j1 > m.buts_j2) p1Wins++;
                    else if (m.buts_j1 < m.buts_j2) p2Wins++;
                    else draws++;
                  } else {
                    if (m.buts_j2 > m.buts_j1) p1Wins++;
                    else if (m.buts_j2 < m.buts_j1) p2Wins++;
                    else draws++;
                  }
                });

                // Recent form (last 5 matches)
                const getRecentForm = (player) => {
                  const recentMatches = filteredData
                    .filter(m => m.joueur1 === player || m.joueur2 === player)
                    .sort((a, b) => new Date(b.dateMatch) - new Date(a.dateMatch))
                    .slice(0, 5);

                  let wins = 0;
                  recentMatches.forEach(m => {
                    const isJ1 = m.joueur1 === player;
                    const result = isJ1
                      ? (m.buts_j1 > m.buts_j2 ? 'W' : m.buts_j1 < m.buts_j2 ? 'L' : 'D')
                      : (m.buts_j2 > m.buts_j1 ? 'W' : m.buts_j2 < m.buts_j1 ? 'L' : 'D');
                    if (result === 'W') wins++;
                  });
                  return wins / Math.max(recentMatches.length, 1);
                };

                const p1Form = getRecentForm(selectedVersusPlayer1);
                const p2Form = getRecentForm(selectedVersusPlayer2);
                ...
                if (totalMatches > 0) {
                  const p1H2HProb = p1Wins / totalMatches;
                  const p2H2HProb = p2Wins / totalMatches;
                  p1WinProb = (p1H2HProb * h2hWeight + p1Form * formWeight) * 100;
                  p2WinProb = (p2H2HProb * h2hWeight + p2Form * formWeight) * 100;
                } else {
                  p1WinProb = p1Form * 100;
                  p2WinProb = p2Form * 100;
                }

                const drawProb = Math.max(0, 100 - p1WinProb - p2WinProb);

                return (
                  <div className="space-y-4">
                    <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-6">
                      <h4 className="font-bold text-center mb-4 text-slate-800">Probabilité de victoire</h4>
                      <div className="grid grid-cols-3 gap-4 text-center">
                        <div>
                          <p className="text-3xl font-bold text-blue-600">{p1WinProb.toFixed(1)}%</p>
                          <p className="text-sm text-slate-600 mt-1">{selectedVersusPlayer1}</p>
                        </div>
                        <div>
                          <p className="text-3xl font-bold text-slate-400">{drawProb.toFixed(1)}%</p>
                          <p className="text-sm text-slate-600 mt-1">Nul</p>
                        </div>
                        <div>
                          <p className="text-3xl font-bold text-purple-600">{p2WinProb.toFixed(1)}%</p>
                          <p className="text-sm text-slate-600 mt-1">{selectedVersusPlayer2}</p>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="border border-slate-200 rounded-lg p-4">
                        <p className="text-sm font-medium text-slate-600 mb-2">H2H Direct</p>
                        <p className="text-lg"><span className="font-bold text-blue-600">{p1Wins}</span> - <span className="font-bold text-slate-400">{draws}</span> - <span className="font-bold text-purple-600">{p2Wins}</span></p>
                      </div>
                      <div className="border border-slate-200 rounded-lg p-4">
                        <p className="text-sm font-medium text-slate-600 mb-2">Forme récente (5 derniers)</p>
                        <p className="text-lg">
                          <span className="font-bold text-blue-600">{(p1Form * 100).toFixed(0)}%</span>
                          {' vs '}
                          <span className="font-bold text-purple-600">{(p2Form * 100).toFixed(0)}%</span>
                        </p>
                      </div>
                    </div>
                  </div>
            ...
            {/* Power Ranking ELO */}
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">⚡ Power Ranking (ELO)</h3>
              <p className="text-sm text-slate-600 mb-4">Classement basé sur un système ELO tenant compte de la force de l'adversaire</p>

              {(() => {
                // Initialize ELO ratings
                const eloRatings = {};
                joueurs.forEach(j => eloRatings[j] = 1500);
                ...
                sortedMatches.forEach(match => {
                  const p1 = match.joueur1;
                  const p2 = match.joueur2;

                  // Expected scores
                  const expectedP1 = 1 / (1 + Math.pow(10, (eloRatings[p2] - eloRatings[p1]) / 400));
                  const expectedP2 = 1 / (1 + Math.pow(10, (eloRatings[p1] - eloRatings[p2]) / 400));

                  // Actual scores
                  let actualP1, actualP2;
                  if (match.buts_j1 > match.buts_j2) {
                    actualP1 = 1;
                    actualP2 = 0;
                  } else if (match.buts_j1 < match.buts_j2) {
                    actualP1 = 0;
                    actualP2 = 1;
                  } else {
                    actualP1 = 0.5;
                  ...
                  // Update ratings
                  eloRatings[p1] += K * (actualP1 - expectedP1);
                  eloRatings[p2] += K * (actualP2 - expectedP2);
                });

                // Sort by ELO
                const eloRanking = Object.entries(eloRatings)
                  .sort(([, a], [, b]) => b - a)
                  .map(([joueur, rating], index) => ({ joueur, rating, rank: index + 1 }));

                return (
                  <div className="space-y-3">
                    {eloRanking.map((entry, idx) => (
                      <div key={entry.joueur} className="flex items-center gap-4 p-4 bg-gradient-to-r from-slate-50 to-slate-100 rounded-lg border border-slate-200">
                        <div className={`text-2xl font-bold ${
                          idx === 0 ? 'text-yellow-500' :
                          idx === 1 ? 'text-slate-400' :
                          idx === 2 ? 'text-orange-600' : 'text-slate-500'
                        }`}>
                          #{entry.rank}
                        </div>
                        <div className="flex-1">
                          <p className="font-bold text-slate-800">{entry.joueur}</p>
                        </div>
                        <div className="text-right">
                          <p className="text-2xl font-bold text-purple-600">{Math.round(entry.rating)}</p>
            ...
            {/* Clutch Stats */}
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">💎 Statistiques Clutch</h3>
              <p className="text-sm text-slate-600 mb-4">Performances dans les matchs décisifs (finales, matchs serrés)</p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {joueurs.map(joueur => {
                  // Matches where goal difference was ≤1
                  const closeMatches = filteredData.filter(m => {
                    if (m.joueur1 !== joueur && m.joueur2 !== joueur) return false;
                    return Math.abs(m.buts_j1 - m.buts_j2) <= 1;
                  });

                  let closeWins = 0;
                  closeMatches.forEach(m => {
                    const isJ1 = m.joueur1 === joueur;
                    const won = isJ1 ? m.buts_j1 > m.buts_j2 : m.buts_j2 > m.buts_j1;
                    if (won) closeWins++;
                  });

                  const clutchRate = closeMatches.length > 0 ? (closeWins / closeMatches.length * 100) : 0;

                  // Championship finals (last match of each championship)
                  const finalMatches = [];
                  const championships = {};
                  filteredData.forEach(m => {
                    const key = `${m.ligue}-${m.championnat}`;
                    if (!championships[key] || new Date(m.dateMatch) > new Date(championships[key].dateMatch)) {
                      championships[key] = m;
                    }
                  });

                  Object.values(championships).forEach(m => {
                    if (m.joueur1 === joueur || m.joueur2 === joueur) {
                      finalMatches.push(m);
                    }
                  });
                    ...
                    <div key={joueur} className="border border-slate-200 rounded-lg p-4">
                      ...
                      <div className="space-y-3">
                        <div className="flex justify-between items-center">
                          <span className="text-sm text-slate-600">Matchs serrés (≤1 but)</span>
                          <span className="font-bold text-purple-600">{closeWins}/{closeMatches.length}</span>
                        </div>
                        <div className="w-full bg-slate-200 rounded-full h-2">
                          <div className="bg-purple-600 h-2 rounded-full" style={{ width: `${clutchRate}%` }}></div>
                        </div>
                        <div className="flex justify-between items-center pt-2">
                          <span className="text-sm text-slate-600">Victoires en finale</span>
                          <span className="font-bold text-yellow-600">{finalWins}/{finalMatches.length}</span>
                        </div>
                      </div>
                    </div>
                  );
            ...
            {/* Trend Analysis */}
            <div className="bg-white rounded-xl shadow-sm p-6">
              <h3 className="text-xl font-bold text-slate-800 mb-4">📈 Analyse des Tendances</h3>
              <p className="text-sm text-slate-600 mb-4">Performance par ligue et évolution temporelle</p>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {joueurs.map(joueur => {
                  ...
                  const byLigue = {};
                  ligues.forEach(ligue => {
                    const ligueMatches = filteredData.filter(m =>
                      m.ligue === ligue && (m.joueur1 === joueur || m.joueur2 === joueur)
                    );

                    let wins = 0;
                    ligueMatches.forEach(m => {
                      const isJ1 = m.joueur1 === joueur;
                      const won = isJ1 ? m.buts_j1 > m.buts_j2 : m.buts_j2 > m.buts_j1;
                      if (won) wins++;
                    });

                    if (ligueMatches.length > 0) {
                      byLigue[ligue] = {
                        winRate: (wins / ligueMatches.length * 100).toFixed(1),
                        total: ligueMatches.length
                      };
                    }
                  });
                  ...
                  return (
                    <div key={joueur} className="border border-slate-200 rounded-lg p-4">
                      <h4 className="font-bold text-lg text-slate-800 mb-3">{joueur}</h4>
                      <div className="space-y-2">
                        {Object.entries(byLigue).length > 0 ? (
                          Object.entries(byLigue).map(([ligue, stats]) => (
                            <div key={ligue} className="flex items-center gap-3">
                              <div className="flex-1">
                                <p className="text-sm font-medium text-slate-700">{ligue}</p>
                              ...
                              <div className="text-right">
                                <p className="text-sm font-bold text-purple-600">{stats.winRate}%</p>
                                <p className="text-xs text-slate-500">{stats.total} matchs</p>
                              </div>
                            </div>
                          ))
                        ) : (
